...
describe("RestaurantSeedSchema", () => {
  ...
  it("rejects a booking link on a platform other than the one declared", () => {
    // The single most likely data-entry slip: pick RESY from the list, paste a Tock link.
    expect(
      problemPaths(
  ...
  it("rejects an unknown platform", () => {
    expect(problemPaths(entry({}, { platform: "SEATED" }))).toContain("releaseRule.platform");
  });
});
