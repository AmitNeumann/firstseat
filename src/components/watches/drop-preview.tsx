 ...
 * database when the form is submitted. A dining day that has already passed is a
 * different refusal from a future day whose booking window just opened.
 */
export function DropPreview({
  ...
  const restaurantZone = restaurant.rules[0]?.timezone ?? userZone;
  const pastDiningDate =
    clockReady && diningDateIsPast(targetDate, new Date(now), restaurantZone);
  const windowAlreadyOpened =
    clockReady &&
    !pastDiningDate &&
    moments.every(({ moment }) => moment.dropDatetime.getTime() <= now);
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border bg-background p-3">
      ...
      {pastDiningDate && (
        <FormAlert tone="error">{PAST_DINING_DATE_MESSAGE}</FormAlert>
      )}

      {windowAlreadyOpened && (
        <FormAlert tone="error">{BOOKING_WINDOW_OPENED_PREVIEW_MESSAGE}</FormAlert>
      )}
